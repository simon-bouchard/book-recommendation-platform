...
class ExecutionLimits:
    ...
    def is_within_limits(self, iterations: int, elapsed_seconds: float, 
                        tool_calls: int, reasoning_steps: int) -> bool:
        """Check if execution is within all limits."""
        return (
            iterations <= self.max_iterations and
            elapsed_seconds <= self.timeout_seconds and
            tool_calls <= self.max_tool_calls and
            reasoning_steps <= self.max_reasoning_steps
        )