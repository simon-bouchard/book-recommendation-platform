...
from .interfaces import ResultProcessor
...
class StandardResultProcessor:
    ...
    def _extract_from_return_book_ids_tool(self, state: AgentExecutionState) -> BookExtractionResult:
            ...
            if execution.tool_name.lower() == "return_book_ids" and execution.succeeded:
                ...
                if isinstance(execution.arguments, dict):
                    if "book_ids" in execution.arguments:
                        try:
                            ids = [int(x) for x in execution.arguments["book_ids"] if str(x).strip()]
                            return BookExtractionResult(
                                book_ids=ids,
    ...
    def _extract_from_tool_outputs(self, state: AgentExecutionState) -> BookExtractionResult:
        ...
        return BookExtractionResult(
            book_ids=unique_ids,
            extraction_method="tool_outputs"
        )
    ...
    def _extract_from_reasoning_steps(self, state: AgentExecutionState) -> BookExtractionResult:
        ...
        return BookExtractionResult(
            book_ids=unique_ids,
            extraction_method="reasoning_steps"
        )
    
    def _extract_from_intermediate_outputs(self, state: AgentExecutionState) -> BookExtractionResult:
        ...
        return BookExtractionResult(
            book_ids=unique_ids,
            extraction_method="intermediate_outputs"
        )
