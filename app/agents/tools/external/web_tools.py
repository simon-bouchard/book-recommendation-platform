...
class WebTools:
    ...
    def _create_web_search_tool(self) -> ToolDefinition:
        ...
        return tool(
            name="web_search",
            description="Search the web for current information, news, lists, or comparisons",
            category=ToolCategory.WEB,
            is_deterministic=False,
        )(web_search)
    
    def _create_wikipedia_tool(self) -> ToolDefinition:
    ...
    def _create_openlibrary_search_tool(self) -> ToolDefinition:
        ...
        def openlibrary_search(query: str) -> str:
            ...
            try:
                ...
                for doc in docs:
                    ...
                    lines.append(
                        f"- {title} — {author}{meta_str}  "
                        f"[openlibrary.org{work_key}]"
                    )
        ...
        return tool(
            ...
            description="Search OpenLibrary database for real books with metadata",
            category=ToolCategory.WEB,
            is_deterministic=False,
        )(openlibrary_search)
    
    def _create_openlibrary_work_tool(self) -> ToolDefinition:
        ...
        def openlibrary_work(work_key: str) -> str:
            ...
            if normalized in self.state.seen_ol_works:
                return "[Guardrail] This OpenLibrary work was already fetched. Use existing results."
        ...
        return tool(
            name="openlibrary_work",
            description="Fetch detailed info for a specific OpenLibrary work by ID",
            category=ToolCategory.WEB,
            is_deterministic=False,
        )(openlibrary_work)