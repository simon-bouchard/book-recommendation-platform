...
from app.table_models import Interaction, Subject
...
def _build_search_request(
    ...
    if subjects:
        ...
        subject_rows = db.query(Subject.subject_idx).filter(Subject.subject.in_(subject_list)).all()
    ...
    def _update():
        try:
            ...
            avg_result = (
                ...
                .filter(Interaction.item_idx == book_id, Interaction.rating.isnot(None))
                .scalar()
            )

            count_result = (
                db.query(func.count(Interaction.rating))
                .filter(Interaction.item_idx == book_id, Interaction.rating.isnot(None))
                .scalar()
            )
            ...
            logger.info(
                f"Updated Meilisearch ratings for book {book_id}: {num_ratings} ratings, avg {avg_rating}"
            )
