    ...
    with SessionLocal() as db:
        ...
        for book in books:
            ...
            if include_metadata:
                ...
                if tags_version == "v1":
                    text = build_v1_subjects_text(title=title, author=author, subjects=subjects)
                else:  # v2
                    text = build_v2_subjects_text(title=title, author=author, subjects=subjects)
...
Examples:
  ...
  python -m app.semantic_index.builders.build_enriched_index --tags-version v1 --full --limit 100 --output test_output
    ...
    parser.add_argument(
        "--tags-version", required=True, choices=["v1", "v2"], help="Enrichment version (v1 or v2)"
    )
