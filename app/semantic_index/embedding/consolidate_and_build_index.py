...
def remove_duplicates(
    embeddings: np.ndarray, item_indices: np.ndarray, metadata: List[Dict]
...
""",
    ...
    parser.add_argument(
        "--accumulator-dir", required=True, help="Directory containing batch_*.npz files"
    )
    parser.add_argument("--output-dir", required=True, help="Output directory for FAISS index")
    parser.add_argument("--tags-version", default="v2", help="Tags version (for metadata)")
    parser.add_argument(
        "--embed-version", default="full_v1", help="Embedding version (for metadata)"
    )
    ...
    # Save index
    try:
        save_index(output_dir, index, item_indices, metadata, args.tags_version, args.embed_version)
    except Exception as e:
