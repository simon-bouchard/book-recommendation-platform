...
class EmbeddingWorker:
    ...
    def _identify_items_to_embed(self, items: List[dict]) -> List[dict]:
        ...
        existing_fingerprints = self.fingerprints.get_existing_fingerprints(
            item_indices, self.tags_version
        )
    ...
    def run_full_load(self, limit: Optional[int] = None):
        ...
        try:
            ...
            while offset < total_items:
                ...
                # Fetch chunk
                items = fetcher.fetch_enriched_items(limit=chunk_size, offset=offset)
    ...
    def run_incremental(self, limit: Optional[int] = None):
        ...
        try:
            ...
            items = fetcher.fetch_enriched_items(limit=limit, offset=0)
...
def main():
    ...
    parser.add_argument(
        "--tags-version", default="v2", help="Tags version to process (default: v2)"
    )

    parser.add_argument(
        "--batch-size", type=int, default=128, help="Batch size for embedding (default: 128)"
    )

    parser.add_argument(
        "--limit", type=int, default=None, help="Max items to process (default: all)"
    )

    parser.add_argument(
        "--output-dir", default="models/data/enriched_v2", help="Output directory for embeddings"
    )

    parser.add_argument(
        "--ontology-dir", default="ontology", help="Directory containing ontology CSV files"
    )

    parser.add_argument(
        "--device", default="cpu", choices=["cpu", "cuda", "mps"], help="Device for embedding model"
    )
