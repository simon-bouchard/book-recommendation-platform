...
def validate_payload(
    ...
    if subj_required and subject_count == 0:
        raise ValueError(f"[{tier}] subjects cannot be empty (minimum: {subj_min})")

    if subject_count < subj_min:
        raise ValueError(f"[{tier}] subjects count ({subject_count}) below minimum ({subj_min})")

    if subject_count > subj_max:
        raise ValueError(f"[{tier}] subjects count ({subject_count}) exceeds maximum ({subj_max})")

    # Check for non-empty strings
    if any(not s or not s.strip() for s in payload.subjects):
        raise ValueError(f"[{tier}] subjects contain empty strings")
    ...
    if tone_required and tone_count == 0:
        raise ValueError(f"[{tier}] tone_ids cannot be empty (minimum: {tone_min})")

    if tone_count < tone_min:
        raise ValueError(f"[{tier}] tone_ids count ({tone_count}) below minimum ({tone_min})")

    if tone_count > tone_max:
        raise ValueError(f"[{tier}] tone_ids count ({tone_count}) exceeds maximum ({tone_max})")
    ...
    if reqs["genre"]["required"]:
        ...
        if payload.genre_id not in valid_genre_ids:
            raise ValueError(
                f"[{tier}] Invalid genre_id: {payload.genre_id}. Must be from valid genre list."
            )

        # Also validate that genre slug is present (for database storage)
        if not payload.genre or not payload.genre.strip():
            raise ValueError(f"[{tier}] genre slug is required but missing or empty")
    ...
    if not payload.vibe or not payload.vibe.strip():
        if vibe_required:
            raise ValueError(f"[{tier}] vibe is required but empty")
        ...
        if tier in ("MINIMAL", "BASIC") and payload.vibe.strip():
            raise ValueError(f"[{tier}] vibe must be empty (got: '{payload.vibe[:50]}...')")
...
def _check_subject_uniqueness(subjects: List[str], tier: str) -> None:
    ...
    for s in subjects:
        ...
        if s_lower in seen:
            ...
            raise ValueError(f"[{tier}] Duplicate subject detected: '{s}'")
        seen.add(s_lower)
