...
from app.table_models import Author, Book, BookSubject, Interaction, Subject, User, UserFavSubject
...
def main():
    ...
    try:
        ...
        # === 4. Import Subjects ===
        book_sub_df = pd.read_csv("data/books_to_subjects.csv").dropna(
            subset=["work_id", "subjects"]
        )
        user_sub_df = pd.read_csv("data/users_to_subjects.csv").dropna(
            subset=["user_id", "favorite_subjects"]
        )
        ...
        subject_counts = Counter(s for s in book_sub_df["subjects"] if s not in excluded_subjects)
        ...
        books = []
        for row in tqdm(
            book_df.itertuples(index=False), total=len(book_df), desc="Preparing books"
        ):
        ...
        for i in tqdm(range(0, len(user_sub_df), CHUNK), desc="Importing user fav subjects"):
            ...
            favs = [
                UserFavSubject(
                    user_id=row.user_id, subject_idx=subject_map.get(row.favorite_subjects)
                )
                for row in chunk.itertuples(index=False)
