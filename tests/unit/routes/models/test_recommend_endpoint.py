...
class TestRecommendEndpointBasics:
    ...
    def test_endpoint_returns_200_with_valid_user_id(
        self, test_client, mock_fetch_user, mock_recommendation_service, monkeypatch
    ):
...
class TestRecommendEndpointUserLookup:
    ...
    def test_lookup_by_user_id_when_id_true(
        ...
        """Should call _fetch_user_and_subjects with _id=True when _id=true."""
        monkeypatch.setattr(
            "routes.models.RecommendationService", lambda: mock_recommendation_service
        )
        ...
        call_args = mock_fetch_user.call_args
        assert call_args[0][1] is True  # _id argument

    def test_lookup_by_username_when_id_false(
        self, test_client, mock_fetch_user, mock_recommendation_service, monkeypatch
    ):
        """Should call _fetch_user_and_subjects with _id=False when _id=false."""
        monkeypatch.setattr(
            "routes.models.RecommendationService", lambda: mock_recommendation_service
        )
        ...
        mock_fetch_user.assert_called_once()
        call_args = mock_fetch_user.call_args
        assert call_args[0][1] is False  # _id argument

    def test_returns_404_when_user_not_found(self, test_client, mock_fetch_user):
...
class TestRecommendEndpointUserConversion:
    """Test user data to domain User conversion."""

    def test_converts_user_with_preferences(
        self, test_client, mock_fetch_user, mock_recommendation_service, monkeypatch
    ):
        """Should convert user tuple with favorite subjects to domain User."""
        monkeypatch.setattr(
            "routes.models.RecommendationService", lambda: mock_recommendation_service
        )
    ...
    def test_converts_user_without_preferences_to_pad_idx(
        self,
        test_client,
        mock_user_no_preferences,
