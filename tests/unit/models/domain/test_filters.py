...
class TestReadBooksFilter:
    ...
    async def test_removes_books_user_has_read(
        ...
        filter_obj = ReadBooksFilter()
        filtered = await filter_obj.apply(sample_candidates, mock_user)
    ...
    @pytest.mark.asyncio
    async def test_handles_empty_candidate_list(self, mock_user, monkeypatch):
        ...
        filter_obj = ReadBooksFilter()
        filtered = await filter_obj.apply([], mock_user)
    ...
    async def test_queries_database_with_correct_user_id(
        ...
        filter_obj = ReadBooksFilter()
        await filter_obj.apply(sample_candidates, mock_user)
...
class TestMinRatingCountFilter:
    ...
    @pytest.mark.asyncio
    async def test_preserves_order_of_candidates(self, sample_candidates, mock_user):
        """Should maintain original candidate order."""
        with _patch_meta_client(_meta_dict(_DEFAULT_RATINGS)):
            filter_obj = MinRatingCountFilter(min_count=10)
            filtered = await filter_obj.apply(sample_candidates, mock_user)
    ...
    @pytest.mark.asyncio
    async def test_handles_missing_books_in_metadata(self, mock_user):
        ...
        with _patch_meta_client({}):  # enrich returns nothing for unknown book
            ...
            filter_obj = MinRatingCountFilter(min_count=10)
            filtered = await filter_obj.apply(candidates, mock_user)

        assert filtered == []

    @pytest.mark.asyncio
    async def test_min_count_zero_keeps_all(self, sample_candidates, mock_user):
...
class TestFilterChain:
    ...
    @pytest.mark.asyncio
    async def test_applies_filters_in_sequence(self, sample_candidates, mock_user):
    ...
    @pytest.mark.asyncio
    async def test_empty_chain_returns_all_candidates(self, sample_candidates, mock_user):
    ...
    @pytest.mark.asyncio
    async def test_single_filter_chain_works(self, sample_candidates, mock_user):
...
class TestNoFilter:
    ...
    @pytest.mark.asyncio
    async def test_returns_all_candidates_unchanged(self, sample_candidates, mock_user):
        """Should return all candidates without modification."""
        filter_obj = NoFilter()
        result = await filter_obj.apply(sample_candidates, mock_user)

        assert result == sample_candidates
        assert result is sample_candidates

    @pytest.mark.asyncio
    async def test_handles_empty_candidates(self, mock_user):
        """Should handle empty candidate list."""
        filter_obj = NoFilter()
        result = await filter_obj.apply([], mock_user)
