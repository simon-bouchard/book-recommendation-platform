...
def check_web_search_usage(tool_executions: List) -> Dict[str, Any]:
    ...
    """
    web_search_calls = [exec for exec in tool_executions if exec.tool_name == "web_search"]

    if not web_search_calls:
        return {"tool_used": False, "reason": "No web_search tool calls made"}

    return {"tool_used": True, "reason": f"Made {len(web_search_calls)} web_search call(s)"}


def judge_answer_quality(
    query: str, response_text: str, recency_expected: bool, judge_llm
) -> Dict[str, Any]:
    """
...
async def evaluate(test_cases: List[Dict], agent: WebAgent) -> Dict[str, Any]:
    """Evaluate web agent on test cases using streaming."""
    results = []
    judge_llm = agent.llm
    ...
    for test in test_cases:
        ...
        if tool_required:
            overall_pass = (
                agent_success and tool_check["tool_used"] and quality_check["quality_pass"]
            )
            ...
            overall_pass = agent_success and quality_check["quality_pass"]
    ...
    return {
        ...
        "tool_required_pass_rate": tool_required_passed / len(tool_required_tests)
        if tool_required_tests
        else 0,
        "tool_optional_pass_rate": tool_optional_passed / len(tool_optional_tests)
        if tool_optional_tests
        else 0,
        "breakdown": {
...
def print_results(eval_results: Dict[str, Any]):
    ...
    print("\n" + "=" * 70)
    print("WEB AGENT EVALUATION (STREAMING)")
    print("=" * 70)
    ...
    print(
        f"	Tool-Optional: {eval_results['tool_optional_pass_rate']:.1%} ({breakdown['tool_optional_passed']}/{breakdown['tool_optional_tests']})"
    )
    ...
    for i, r in enumerate(eval_results["results"], 1):
        ...
        print(
            f"	 {qual_icon} Layer 2 (Quality): Rel={r['relevance']}, Rec={r['recency']}{recency_note}"
        )
...
def main():
    ...
    print("Running evaluation with streaming...\n")
    eval_results = asyncio.run(evaluate(test_cases, agent))
