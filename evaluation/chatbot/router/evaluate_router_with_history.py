...
async def evaluate_with_history(
    test_data: Dict[str, List[Dict]], router: RouterLLM
) -> Dict[str, Any]:
    """
    ...
    for category, test_cases in test_data.items():
        ...
        for test_case in test_cases:
            ...
            router_input = make_router_input(history=history, user_text=final_query, k_user=2)

            # Classify
            plan = await router.classify(router_input)
            actual = plan.target
    ...
    return {
        ...
        "overall": {"correct": total_correct, "total": total_queries, "accuracy": overall_accuracy},
...
async def main():
    ...
    eval_results = await evaluate_with_history(test_data, router)
