...
class EvalDashboard:
    ...
    def parse_result_file(self, agent_name: str, file_path: Path) -> Optional[AgentResult]:
        ...
        try:
            ...
            if not overall:
                ...
                overall = {k: v for k, v in data.items() 
                          if k in ['passed', 'total', 'correct', 'pass_rate', 'accuracy',
                                   'retrieval_pass_rate', 'quality_pass_rate', 'avg_quality_score',
                                   'tool_pass_rate']}
    ...
    def print_agent_summary(self, result: AgentResult, previous: Optional[AgentResult] = None):
        ...
        change_str = ""
        if previous:
            prev_rate = previous.passed_tests / previous.total_tests if previous.total_tests > 0 else 0
        ...
        if result.category_stats:
            ...
            for category, stats in sorted(result.category_stats.items()):
                if isinstance(stats, dict):
                    ...
                    print(f"   {category:30s} {cat_passed:2d}/{cat_total:2d} {self.format_percentage(cat_rate)}{prev_change}")
        
        # Additional metrics (for specific agents)
        if 'retrieval_pass_rate' in result.overall_stats:
            print(f"\n📚 Retrieval Metrics:")
            print(f"   Retrieval Pass Rate: {self.format_percentage(result.overall_stats['retrieval_pass_rate'])}")
            print(f"   Quality Pass Rate:   {self.format_percentage(result.overall_stats['quality_pass_rate'])}")
            print(f"   Avg Quality Score:   {result.overall_stats.get('avg_quality_score', 0):.2f}/2")
        
        if 'tool_pass_rate' in result.overall_stats:
            print(f"\n🔧 Tool Usage Metrics:")
            print(f"   Tool Pass Rate:    {self.format_percentage(result.overall_stats['tool_pass_rate'])}")
            print(f"   Quality Pass Rate: {self.format_percentage(result.overall_stats['quality_pass_rate'])}")
            ...
            for i, test in enumerate(result.failed_tests[:10], 1):  # Show first 10
                ...
                if not reasons:
                    if 'reasoning' in test:
                        ...
                        if all_passed:
                            reasons.append("Test configuration issue or undocumented failure reason")
                        else:
    ...
    def print_summary_table(self, results: Dict[str, Tuple[Optional[AgentResult], Optional[AgentResult]]]):
        ...
        for agent_name, (current, _) in results.items():
            if current:
                pass_rate = current.passed_tests / current.total_tests if current.total_tests > 0 else 0
    ...
    def run(self, show_comparison: bool = True, verbose: bool = False):
        ...
        for agent_name, files in latest_files.items():
            ...
            previous = self.parse_result_file(agent_name, files[1]) if len(files) > 1 and show_comparison else None
...
Examples:
  ...
  %(prog)s -p /path/to/evaluation   # Use custom base path
    ...
    parser.add_argument(
        '-v', '--verbose',
        action='store_true',
        help='Show detailed breakdown for each agent'
    )
    
    parser.add_argument(
        '--no-comparison',
        action='store_true',
        help='Do not compare with previous evaluation runs'
    )
