...
class BasePooler(nn.Module):
    ...
    def __init__(self, n_users: int, n_items: int, n_subjects: int, emb_dim: int, dropout: float = 0.1):
        super().__init__()
    ...
    def rating_head(self, u_emb: torch.Tensor, i_emb: torch.Tensor, u_idx: torch.Tensor, i_idx: torch.Tensor) -> torch.Tensor:
        dot = (u_emb * i_emb).sum(dim=1)
        return dot + self.user_bias(u_idx).squeeze() + self.item_bias(i_idx).squeeze() + self.global_bias
...
class ScalarPooler(BasePooler):
    def __init__(self, n_users: int, n_items: int, n_subjects: int, emb_dim: int = 16, dropout: float = 0.3):
...
class SelfAttentionBlock(nn.Module):
    def __init__(self, d_model: int = 64, n_heads: int = 4, attn_drop: float = 0.1, ffn_mult: int = 2, ffn_drop: float = 0.1):
        super().__init__()
...
class SelfAttentionPooler(BasePooler):
    def __init__(self, n_users: int, n_items: int, n_subjects: int, emb_dim: int = 64, n_heads: int = 4, dropout: float = 0.1):
        super().__init__(n_users, n_items, n_subjects, emb_dim, dropout)
...
class SelfAttnPerDimPooler(BasePooler):
    def __init__(self, n_users: int, n_items: int, n_subjects: int, emb_dim: int = 16, n_heads: int = 2, dropout: float = 0.1):
        super().__init__(n_users, n_items, n_subjects, emb_dim, dropout)
        self.subject_emb = nn.Embedding(n_subjects, emb_dim, padding_idx=PAD_IDX)
...
def build_pooler(kind: PoolerKind = None,
                 n_users: int = 1,
                 n_items: int = 1,
                 n_subjects: int = 1,
                 emb_dim: int = 16,
                 dropout: float = 0.1,
                 n_heads: int = 4) -> BasePooler:
    ...
    if k == "selfattn":
        ...
        return SelfAttentionPooler(n_users, n_items, n_subjects, emb_dim=emb_dim, n_heads=n_heads, dropout=dropout)
    if k == "selfattn_perdim":
        return SelfAttnPerDimPooler(n_users, n_items, n_subjects, emb_dim=emb_dim, n_heads=n_heads, dropout=dropout)
    raise ValueError(f"Unknown attention kind: {kind}")


def build_pooler_from_env(n_users: int,
                          n_items: int,
                          n_subjects: int,
                          emb_dim: int = None,
                          dropout: float = None,
                          n_heads: int = None) -> Tuple[BasePooler, PoolerKind]:
    """
    ...
    pooler = build_pooler(kind, n_users=n_users, n_items=n_items, n_subjects=n_subjects,
                          emb_dim=emb_dim, dropout=dropout, n_heads=n_heads)
