...
def analyze_description_coverage(db: Session) -> Dict:
    ...
    print(
        f"Books with VALID description: {has_valid_desc:,} ({has_valid_desc / total_books * 100:.1f}%)"
    )
    print(
        f"Books with placeholder text: {placeholder_count:,} ({placeholder_count / total_books * 100:.1f}%)"
    )
    print(f"Books with null/empty: {null_or_empty:,} ({null_or_empty / total_books * 100:.1f}%)")
    print(
        f"Books without usable description: {total_books - has_valid_desc:,} ({(total_books - has_valid_desc) / total_books * 100:.1f}%)"
    )
...
def analyze_ol_subjects_coverage(db: Session) -> Dict:
    ...
    has_ol_subjects = db.query(func.count(func.distinct(BookOLSubject.item_idx))).scalar()

    # Subject count distribution
    subject_counts = (
        db.query(BookOLSubject.item_idx, func.count(BookOLSubject.ol_subject_idx).label("count"))
        .group_by(BookOLSubject.item_idx)
        .all()
    )
    ...
    print(f"\nTotal books: {total_books:,}")
    print(
        f"Books with OL subjects: {has_ol_subjects:,} ({has_ol_subjects / total_books * 100:.1f}%)"
    )
    print(
        f"Books without OL subjects: {total_books - has_ol_subjects:,} ({(total_books - has_ol_subjects) / total_books * 100:.1f}%)"
    )
    ...
    if counts_list:
        ...
        top_subjects = (
            db.query(OLSubject.subject, func.count(BookOLSubject.item_idx).label("book_count"))
            .join(BookOLSubject)
            .group_by(OLSubject.subject)
            .order_by(func.count(BookOLSubject.item_idx).desc())
            .limit(20)
            .all()
        )
...
def analyze_basic_metadata(db: Session) -> Dict:
    ...
    # Title coverage
    has_title = (
        db.query(func.count(Book.item_idx))
        .filter(Book.title.isnot(None), Book.title != "")
        .scalar()
    )

    # Author coverage
    has_author = db.query(func.count(Book.item_idx)).filter(Book.author_idx.isnot(None)).scalar()

    # Both title and author
    has_both = (
        db.query(func.count(Book.item_idx))
        .filter(Book.title.isnot(None), Book.title != "", Book.author_idx.isnot(None))
        .scalar()
    )
    ...
    print(
        f"Books missing title or author: {total_books - has_both:,} ({(total_books - has_both) / total_books * 100:.1f}%)"
    )
...
def simulate_tier_assignment(db: Session, desc_stats: Dict, ol_stats: Dict) -> Dict:
    ...
    books = (
        ...
        .outerjoin(BookOLSubject, Book.item_idx == BookOLSubject.item_idx)
        .group_by(Book.item_idx, Book.title, Book.description, Book.author_idx)
        .all()
    )
    ...
    for tier in ["RICH", "SPARSE", "MINIMAL", "BASIC", "INSUFFICIENT"]:
        ...
        if tier in tier_score_ranges and tier_score_ranges[tier]:
            ...
            score_info = (
                f"  [scores: {min(scores)}-{max(scores)}, avg: {sum(scores) / len(scores):.1f}]"
            )
    ...
    for tier in ["RICH", "SPARSE", "MINIMAL", "BASIC", "INSUFFICIENT"]:
        if tier_examples[tier]:
            ...
            for ex in tier_examples[tier]:
                print(
                    f"  item_idx={ex['item_idx']:>6} | score={ex.get('score', 0):>3} | "
...
def analyze_coverage_overlap(db: Session) -> Dict:
    ...
    all_books = (
        ...
        .outerjoin(BookOLSubject, Book.item_idx == BookOLSubject.item_idx)
        .group_by(Book.item_idx, Book.description)
        .all()
    )
    ...
    print(
        f"  Only description (no OL subjects):   {only_desc:>8,} ({only_desc / total_books * 100:>5.1f}%)"
    )
    print(
        f"  Only OL subjects (no description):   {only_ol:>8,} ({only_ol / total_books * 100:>5.1f}%)"
    )
    print(
        f"  Neither description nor OL subjects: {neither:>8,} ({neither / total_books * 100:>5.1f}%)"
    )
...
def main():
    ...
    print(
        f"  RICH (score >= 60):    {rich_count:>8,} ({rich_count / total * 100:>5.1f}%) - Full enrichment"
    )
    print(
        f"  SPARSE (30-59):        {sparse_count:>8,} ({sparse_count / total * 100:>5.1f}%) - Focused enrichment"
    )
    print(
        f"  MINIMAL (10-29):       {minimal_count:>8,} ({minimal_count / total * 100:>5.1f}%) - Basic enrichment"
    )
    print(
        f"  BASIC (< 10):          {basic_count:>8,} ({basic_count / total * 100:>5.1f}%) - Genre only"
    )

    print("\nMetadata Signal Analysis:")
    print(
        f"  Books with valid descriptions: {desc_stats['has_valid_description']:,} ({desc_stats['has_valid_description'] / total * 100:.1f}%)"
    )
    print(
        f"  Books with OL subjects: {ol_stats['has_ol_subjects']:,} ({ol_stats['has_ol_subjects'] / total * 100:.1f}%)"
    )
    print(
        f"  Books with BOTH signals: {overlap_stats['both']:,} ({overlap_stats['both'] / total * 100:.1f}%)"
    )
    print(
        f"  Books with ONLY OL subjects: {overlap_stats['only_ol_subjects']:,} ({overlap_stats['only_ol_subjects'] / total * 100:.1f}%)"
    )
    print(
        f"  Books with NEITHER signal: {overlap_stats['neither']:,} ({overlap_stats['neither'] / total * 100:.1f}%)"
    )
    ...
    if high_quality > low_quality:
        print(
            f"  ✓ {high_quality:,} books ({high_quality / total * 100:.1f}%) in RICH/SPARSE tiers"
        )
        print("    → Good metadata coverage, tiering will improve quality")
    else:
        print(
            f"  ⚠ {low_quality:,} books ({low_quality / total * 100:.1f}%) in MINIMAL/BASIC tiers"
        )
        print("    → Many books have sparse metadata, conservative approach crucial")

    # Insight 2: BASIC tier size
    if basic_count > 0.15 * total:
        print(f"  ⚠ BASIC tier is large ({basic_count / total * 100:.1f}%)")
        print("    → Many books will get genre-only enrichment")
        print("    → Consider if threshold adjustment needed after testing")
    elif basic_count > 0.05 * total:
        print(f"  ✓ BASIC tier is reasonable ({basic_count / total * 100:.1f}%)")
        print("    → Acceptable for very sparse metadata books")
    else:
        print(f"  ✓ BASIC tier is small ({basic_count / total * 100:.1f}%)")
        print("    → Most books have enough metadata for subject extraction")
    ...
    if overlap_stats["only_ol_subjects"] > 0.1 * total:
        ...
        print("    → Combined scoring prevents these from falling to BASIC tier")
    ...
    if "tier_score_ranges" in tier_stats:
        for tier in ["RICH", "SPARSE", "MINIMAL", "BASIC"]:
            if tier in tier_stats["tier_score_ranges"] and tier_stats["tier_score_ranges"][tier]:
                ...
                print(
                    f"  → {tier}: avg score = {avg_score:.1f} (range: {min(scores)}-{max(scores)})"
                )
