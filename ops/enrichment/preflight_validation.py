...
from app.table_models import Book, BookOLSubject, Genre, OLSubject, Tone
...
def check_tones_data() -> Tuple[bool, str]:
    ...
    try:
        with SessionLocal() as db:
            ...
            v1_count = db.query(Tone).filter(Tone.ontology_version == "v1").count()

            if v1_count == 0:
                return False, "Tones v1: no tones found (need at least some v1 tones)"

            # Check v2 (required for Phase 2)
            v2_count = db.query(Tone).filter(Tone.ontology_version == "v2").count()

            if v2_count != 36:
                return False, f"Tones v2: expected 36, found {v2_count} (Phase 2 requires v2)"

            # Check ID ranges
            v1_ids = db.query(Tone.tone_id).filter(Tone.ontology_version == "v1").all()
            v1_min = min(id for (id,) in v1_ids)
            v1_max = max(id for (id,) in v1_ids)

            v2_ids = db.query(Tone.tone_id).filter(Tone.ontology_version == "v2").all()
...
def check_genres_data() -> Tuple[bool, str]:
    ...
    try:
        with SessionLocal() as db:
            v1_count = db.query(Genre).filter(Genre.ontology_version == "v1").count()
...
def check_books_data() -> Tuple[bool, str]:
    ...
    try:
        with SessionLocal() as db:
            ...
            with_idx = db.query(Book).filter(Book.item_idx.isnot(None)).count()
...
def check_validator() -> Tuple[bool, str]:
    ...
    try:
        ...
        with SessionLocal() as db:
            tone_ids = {
                t.tone_id
                for t in db.query(Tone).filter(Tone.ontology_version == "v2").limit(5).all()
            }

            genre_slugs = {
                g.slug
                for g in db.query(Genre).filter(Genre.ontology_version == "v1").limit(5).all()
            }
        ...
        try:
            validate_payload(
                payload={"subjects": [], "tone_ids": [], "genre": list(genre_slugs)[0], "vibe": ""},
                valid_tone_ids=tone_ids,
                valid_genre_slugs=genre_slugs,
                tier="BASIC",
            )
...
def check_existing_enrichment() -> Tuple[bool, str]:
    ...
    try:
        with SessionLocal() as db:
            ...
            v2_count = (
                db.query(BookTone.item_idx).filter(BookTone.tags_version == "v2").distinct().count()
            )
...
def run_all_checks(skip_slow: bool = False):
    ...
    print(f"\n{BLUE}{'=' * 70}")
    print("PRE-FLIGHT VALIDATION FOR PHASE 2 ENRICHMENT")
    print(f"{'=' * 70}{RESET}\n")
    ...
    if passed_count == total_count:
        ...
        print("\nNext steps:")
        print(
            "  1. Start Spark consumer: docker-compose -f docker/spark-loader/docker-compose.yml up -d"
        )
        print("  2. Run enrichment: python -m app.enrichment.runner_kafka --limit 100")
        return 0
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Pre-flight validation for Phase 2 enrichment")
    parser.add_argument(
        "--quick", action="store_true", help="Skip slower checks (classifier, validator tests)"
    )
