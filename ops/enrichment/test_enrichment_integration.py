...
class InterceptingProducer(EnrichmentProducer):
    ...
    def send_result(
        self, item_idx, subjects, tone_ids, genre, vibe, tags_version, scores=None, metadata=None
    ) -> bool:
        # Capture the event
        event = {
            "item_idx": item_idx,
        ...
        return super().send_result(
            item_idx, subjects, tone_ids, genre, vibe, tags_version, scores, metadata
        )
...
def run_enrichment_on_books(
    book_ids: List[int], capture: KafkaEventCapture, tags_version: str
) -> Dict[str, int]:
...
def wait_for_consumer(tags_version: str, expected_count: int, timeout: int = CONSUMER_WAIT_SECONDS):
    ...
    with SessionLocal() as db:
        while time.time() - start < timeout:
            ...
            total += (
                db.query(BookLLMSubject).filter(BookLLMSubject.tags_version == tags_version).count()
            )
            total += (
        ...
        for item_idx, kafka_error in capture.errors.items():
            ...
            if not sql_error:
                ...
                if sql_error.error_code != kafka_error["error_code"]:
                    mismatches.append(
                        f"error_code: {kafka_error['error_code']} vs {sql_error.error_code}"
                    )
                ...
                if mismatches:
                    discrepancies.append(
                        {"item_idx": item_idx, "field": "error", "mismatches": mismatches}
                    )
                else:
                    verified["errors"] += 1
...
def main():
    ...
    try:
        ...
        if not consumer_ok:
            logger.warning(
                "⚠️  Consumer may not have finished - verification might show discrepancies"
            )
