...
def notify(status: str, title: str, body: str = "", log_tail: Optional[str] = None):
    if SLACK:
        att = {
            "color": {"start": "#888", "ok": "#2EB67D", "fail": "#E01E5A"}.get(status, "#888"),
            "blocks": [
    ...
    if status == "start" and HC_START:
        _get(HC_START)
    if status == "ok" and HC_SUCCESS:
        _get(HC_SUCCESS)
    if status == "fail" and HC_FAIL:
        _get(HC_FAIL, {"msg": (body or "")[:400]})
