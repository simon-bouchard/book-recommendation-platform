...
def check_partition_balance(topic: str, verbose: bool = False):
    ...
    try:
        ...
        if total < MINIMUM_SAMPLE_SIZE:
            ...
            print(f"   Balance metrics are unreliable with few messages.")
        ...
        if total < MINIMUM_SAMPLE_SIZE:
            print(f"⏳ TOO EARLY: Need {MINIMUM_SAMPLE_SIZE - total:,} more messages for reliable assessment")
        elif cv < 10:
...
def check_spark_checkpoint_offsets(checkpoint_dir: str, topic: str):
    ...
    if not offsets_dir.exists():
        ...
        print(f"     Spark may not have started yet")
        return None
    
    # Find latest offset file
    offset_files = sorted(offsets_dir.glob("*"))
    if not offset_files:
        print(f"  ⚠️  No offset files in checkpoint")
        return None
    ...
    try:
        ...
        for tp in topic_partitions:
            ...
            if committed_offset is None:
                ...
                if end_offset > 0:
                    print(f"  Partition {tp.partition}: Not started (no committed offset, {end_offset:,} messages waiting)")
        ...
        if not has_any_committed:
            ...
            print(f"     This is normal if the consumer hasn't started yet")
        elif total_lag == 0:
