...
class SubjectAnalyzer:
    ...
    def print_basic_stats(self):
        ...
        print(f"  With subjects: {stats['books_with_subjects']:,} ({stats['coverage_pct']:.1f}%)")
    ...
    def print_length_analysis(self):
        ...
        print("\nLength distribution:")
        print(
            f"  Characters: {stats['char_min']}-{stats['char_max']} (avg: {stats['char_avg']:.1f})"
        )
        print(f"  Words: {stats['word_min']}-{stats['word_max']} (avg: {stats['word_avg']:.1f})")

        # Show examples of very long subjects
        self.load_data()
        long_subjects = [
            (s, self._subject_counter[s]) for s in self._subject_counter.keys() if len(s) > 100
        ]
