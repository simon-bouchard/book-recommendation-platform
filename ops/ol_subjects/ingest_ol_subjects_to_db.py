...
from app.table_models import Book, OLSubject, BookOLSubject
...
def insert_ol_subjects(db, all_subjects: set):
    ...
    if not new_subjects:
        print(f"  All subjects already exist")
    else:
...
def insert_book_ol_subject_links(db, records, work_id_map, subject_map):
    ...
    for i in range(0, len(links), batch_size):
        ...
        try:
            values = ", ".join(
                f"({link['item_idx']}, {link['ol_subject_idx']})"
                for link in batch
            )
            ...
            if inserted % 50000 == 0:
                print(f"    Inserted {inserted:,}/{len(links):,} (skipped {duplicates:,} duplicates)")
            ...
            for link in batch:
                try:
                    obj = BookOLSubject(
                        item_idx=link['item_idx'],
                        ol_subject_idx=link['ol_subject_idx']
                    )
                    db.add(obj)
                    db.commit()
                    inserted += 1
